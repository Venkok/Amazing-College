...
class Search {
    // 1. describe and create/initiate our object
    constructor() { // any code inside constructor will be executed as soon as we create new object of this Class
        this.addSearchHTML();
        this.resultsDiv = $("#search-overlay__results");
        this.openButton = $(".js-search-trigger");
        this.closeButton = $(".search-overlay__close");
        this.searchOverlay = $(".search-overlay"); // div ìn the footer
    ...
    typingLogic() { 
        ...
        if (this.searchField.val()) {
            ...
            this.isSpinnerVisible = false; /* when search field is empty spinner doesn't run */
 /*       } 
    ...
    getResults() {
        $.getJSON(universityData.root_url + '/wp-json/university/v1/search?term=' + this.searchField.val(), (results) => { // what URL (1st arg.) returns is passed to function (2nd argument)
            this.resultsDiv.html(`
            <div class="row">
                <div class="one-third">
                <h2 class="search-overlay__section-title">General Information</h2>
...
`).join('')} 
        ...
        /* Old JSON request using when-this Asynchronous requests
        $.when(
            $.getJSON(universityData.root_url + '/wp-json/wp/v2/posts?search=' + this.searchField.val()),// Asynchronous request
            $.getJSON(universityData.root_url + '/wp-json/wp/v2/pages?search=' + this.searchField.val()) // Asynchronous request
            ).then((posts, pages) => { // maps posts and pages to 1 & 2 JSON urls
            var combinedResults = posts[0].concat(pages[0]);
            this.resultsDiv.html(` 
            <h2 class="search-overlay__section-title">General Information</h2>
            ${combinedResults.length ? '<ul class="link-list min-list">' : '<p>No information matches this search</p>'}
            ${combinedResults.map(item => `<li><a href="${item.link}">${item.title.rendered}</a> ${item.type == 'post' ? `by ${item.authorName}` : ''}</li>`).join('')} 
            ${combinedResults.length ? '</ul>' : ''}
        `); // using Template Literal `` (тильда)
        this.isSpinnerVisible = false;
        }, () => {
            this.resultsDiv.html('<p>Unexpected error; please try again.</p>'); // second parametr of "this" - failFilter - An optional function that is called when the Deferred is rejected. 
        }); */
...
/*   }   
    ...
    openOverlay() {
        this.searchOverlay.addClass("search-overlay--active"); // add class to div
    ...
    closeOverlay() {
        this.searchOverlay.removeClass("search-overlay--active"); // remove class from div
    ...
    addSearchHTML() {
        ...
        `); // add HTML to the end of body element
    }
}
...
/* PURE JS CODE (without jQuery) - TRY!:
...
Thanks, I agree I hate jQuery too. Would rather use ES6. Your code works and his doesn't.  To improve on your solution, I'd wrap it into an IIFE.
